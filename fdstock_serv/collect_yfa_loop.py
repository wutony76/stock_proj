          ...
          with self.txnmgr.get_request() as txn:
            with self.lock.reader_lock:
              ...
              for vid in all_villager_ids:
                ...
                dt = now_t - last_t
                print('vid=%s dt=%s' % (vid, dt))
                if dt >= 1.6:
                  active_vids.append(vid)

          if active_vids:

            if len(pending_equity_codes) < 1:
              pending_equity_codes = sorted(eq_codes, key=lambda x: random.random())
            ...
            with self.txnmgr.get_request() as txn:
              with self.lock.writer_lock:
                ...
                for i in xrange(4096):
                  if len(pending_equity_codes) == 0:
                    break
                  if len(active_vids) == 0:
                    break

                  vid = active_vids.pop()
                  eq_code = pending_equity_codes.pop()
                  ...
                  headers = {}
                  rtn = cli.call('SEND_COMMAND_TO', vid, 'HTTP_REQ', [url, 'GET', headers, req_data, 11, save_meta])
